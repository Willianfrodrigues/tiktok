view: conjunto_mesclado {
  ...
  dimension: Campanhas_fort {
    ...
    sql: CASE
      ...
      WHEN ${campaign_name} LIKE '%GARDASIL%' OR ${campaign_name} LIKE '%00929%' OR ${campaign_name} LIKE '%Crossmedia | MSD | Consideracao | Gardasil%' THEN 'MSD Gardasil | 2024'
      WHEN ${campaign_name} LIKE '%EMBAIXADORA%' OR ${campaign_name} LIKE '%00871%'OR ${campaign_name} LIKE '%Crossmedia | MSD | Consideracao | Embaixadora%' THEN 'MSD Embaixadora'
...
#------------------------------------------TESTES------------------------------
  ...
  dimension: impressao_ajustada {
    type: number
    sql: CASE
        WHEN ${TABLE}.CAMPAIGN_NAME LIKE '%PHILCO | META | VIEWS%'
             AND DATE(${TABLE}.DATE) = '2024-12-11' THEN ${TABLE}.IMPRESSIONS - 170000
        WHEN ${TABLE}.CAMPAIGN_NAME LIKE '%PHILCO | META | VIEWS%' THEN ${TABLE}.IMPRESSIONS - 1700000
