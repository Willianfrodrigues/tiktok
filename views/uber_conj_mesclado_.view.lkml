  ...
  dimension: influenciador {
    ...
    sql: CASE
      ...
      WHEN ${ad_name} LIKE '%JANAELE LIMA%' OR ${ad_name} LIKE '%Janaele Lima%' OR ${ad_name} LIKE '%Janaele_Lima%' THEN 'Janaele Lima'
       ...
       WHEN ${ad_name} LIKE '%WELLINTON GONCALVES%' OR ${ad_name} LIKE '%WELL GONCALVES%' THEN 'Wellinton Goncalves'
       WHEN ${ad_name} LIKE '%JENNI TISOVEC%' OR ${ad_name} LIKE '%JENNI TISOVEC%' THEN 'Jenni Tisovec'
       WHEN ${ad_name} LIKE '%VITOR BOURGUIGNON%' OR ${ad_name} LIKE '%Vitor Bourguignon%' THEN 'Vitor Bourguignon'
       WHEN ${ad_name} LIKE '%NICOLAS AVANSINI%' OR ${ad_name} LIKE '%Nicolas Avansini%' THEN 'Nicolas Avansini'
